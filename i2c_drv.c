...
#define I2CDRV_LINUX_BUS0_NAME "BB-I2C0"
...
void I2cDrv_init(i2c_device_t *pdevice)
{
	...
	case I2C_HW_BUS_0:
		bus = I2CDRV_LINUX_BUS0;
		busid = I2CDRV_LINUX_BUS0_NAME;
		break;
	case I2C_HW_BUS_1:
		bus = I2CDRV_LINUX_BUS1;
		busid = I2CDRV_LINUX_BUS1_NAME;
		break;
	case I2C_HW_BUS_2:
		bus = I2CDRV_LINUX_BUS2;
		busid = I2CDRV_LINUX_BUS2_NAME;
		break;
	default:
		assert(false);
	}

	WriteToFile(CAPE_SLOTS, busid);
...
void WriteToFile(char* fileName, char* msg)
{
	...
	//Write to file
	int charWritten = fprintf(file, "%s", msg);

	//Check if write was successful
	if (charWritten <= 0) {
		printf("ERROR: unable to write %s to %s\n", msg, fileName);
	}
